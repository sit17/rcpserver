...
@Transactional
public interface ChannelGroupPriDao extends PagingAndSortingRepository<ChannelGroup, String> {
...
//	@Query("select g from ChannelGroup g where g.groupId like %?1% or g.ownerId in(select a.accId from Account a where a.openId like %?1%) or g.name like %?1%")
